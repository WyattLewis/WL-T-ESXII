...
AddEventHandler('nui_doorlock:newDoorSetup', function(args)
	...
	if not validTypes[doorType] then print(doorType.. ' n’est pas un type de porte valide') return end
	if arg and arg.item == '' and arg.job1 == '' then print('Vous devez entrer une tâche ou un élément pour l’autorisation de verrouillage') return end
	if args[7] then print('Vous ne pouvez définir que quatre tâches autorisées - si vous en souhaitez d’autres, ajoutez-les ultérieurement à la configuration') return end
	if doorType == 'door' or doorType == 'sliding' or doorType == 'garage' then
		local entity, coords, heading, model = nil, nil, nil, nil
		local result = false
		print('Visez la porte souhaitée et appuyez sur le bouton gauche de la souris')
		while true do
			Citizen.Wait(0)
		...
		else DrawInfos("Visez la porte souhaitée et tirez") end
			if IsControlJustPressed(0, 24) then break end
		end
		if not model or model == 0 then print('N’a pas reçu de hachage de modèlenSi la porte est transparente, assurez-vous de viser le cadre') return end
		local jobs = {}
		if args[3] then
			jobs[1] = args[3]
			jobs[2] = args[4]
		...
		print('Envoi réussi des données de porte au serveur')
	elseif doorType == 'double' or doorType == 'doublesliding' then
		local entity, coords, heading, model = {}, {}, {}, {}
		local result = false
		print('Visez chaque porte souhaitée et appuyez sur le bouton gauche de la souris')
		while true do
			Citizen.Wait(0)
			if IsPlayerFreeAiming(PlayerId()) then
			...
			if result then DrawInfos("Coords: " .. coords[1] .. "\nHeading: " .. heading[1] .. "\nHash: " .. model[1])
			else DrawInfos("Visez la porte souhaitée et tirez") end
			if IsControlJustPressed(0, 24) then break end
		end
		result = false
		while true do
			...
			if result then DrawInfos("Coords: " .. coords[2] .. "\nHeading: " .. heading[2] .. "\nHash: " .. model[2])
			else DrawInfos("Visez la porte souhaitée et tirez") end
			if IsControlJustPressed(0, 24) then break end
		end
		if not model[1] or model[1] == 0 or not model[2] or model[2] == 0 then print('N’a pas reçu de hachage de modèlenSi la porte est transparente, assurez-vous de viser le cadre') return end
		if entity[1] == entity[2] then print('Impossible d’ajouter une double porte si les entités sont les mêmes') return end
		local jobs = {}
		if args[3] then
			jobs[1] = args[3]
			jobs[2] = args[4]
