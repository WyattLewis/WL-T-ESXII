...
function StartDistressSignal()
	...
	local timer = Config.RespawnDelayAfterRPDeath
	 ...
	 while timer > 0 and IsDead do
        ...
        EndTextCommandDisplayText(0.40, 0.9)
      ...
      if data.current.value == 'ambulance_wear' then
        ...
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
          ...
          if skin.sex == 0 then
			...
			SetPedComponentVariation(GetPlayerPed(-1), 4, 96, 0, 2) --Pantalon Police
			SetPedComponentVariation(GetPlayerPed(-1), 11, 249, 0, 2) --Chemise Police
			SetPedComponentVariation(GetPlayerPed(-1), 3, 94, 0, 0)--Gants
			SetPedComponentVariation(GetPlayerPed(-1), 10, 57, 0, 0)--Grade
			SetPedComponentVariation(GetPlayerPed(-1), 7, 126, 0, 1)--cravate
          end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
          ...
          local plate = 'EMS ' .. math.random(100, 900)
        ...
        if IsPedInAnyVehicle(playerPed,  false) then
          ...
          if IsPedSittingInVehicle(CurrentCustomer,  vehicle) then
            ...
            if CustomerEnteredVehicle then
			  ...
			  TargetCoords = {x = 365.967,y = -570.150,z = 28.791 }
