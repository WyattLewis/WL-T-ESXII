...
AddEventHandler('nui_doorlock:updateState', function(doorID, locked, src, usedLockpick)
	...
	if type(doorID) ~= 'number' then
		print(('nui_doorlock: %s (%s) n\'a pas envoyé de numéro! (Sent %s)'):format(xPlayer.getName(), xPlayer.identifier, doorID))
		return
	end

	if type(locked) ~= 'boolean' then
		print(('nui_doorlock: %s (%s) tentative de mise à jour de l’état non valide! (Sent %s)'):format(xPlayer.getName(), xPlayer.identifier, locked))
		return
	end

	if not Config.DoorList[doorID] then
		print(('nui_doorlock: %s (%s) tentative de mise à jour d’une porte non valide! (Sent %s)'):format(xPlayer.getName(), xPlayer.identifier, doorID))
		return
	end
...
function IsAuthorized(xPlayer, doorID, locked, usedLockpick)
	...
	if not canOpen then
		...
		if group == 'superadmin' or group == 'admin' then
			 print(group..' '..xPlayer.getName..'  a été autorisé à utiliser une porte')
			canOpen = true
		end
	end
...
RegisterCommand('.PapiPorte', function(playerId, args, rawCommand)
...
AddEventHandler('nui_doorlock:newDoorCreate', function(config, model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, doubleDoor, doorname)
	...
	if not IsPlayerAceAllowed(source, 'command.newdoor') then print(xPlayer.getName().. 'a tenté de créer une nouvelle porte, mais n’a pas l’autorisation') return end
